package io.github.treesitter.jtreesitter.zathurarc;

import java.lang.foreign.*;

public final class TreeSitterZathurarc {
    ...
    private static final TreeSitterZathurarc INSTANCE = new TreeSitterZathurarc();

    private final Arena arena = Arena.ofAuto();
    private volatile SymbolLookup lookup = null;

    private TreeSitterZathurarc() {}
    ...
    public static MemorySegment language(SymbolLookup lookup) {
        return call(lookup, "tree_sitter_zathurarc");
    }

    private SymbolLookup findLibrary() {
        try {
            var library = System.mapLibraryName("tree-sitter-zathurarc");
            return SymbolLookup.libraryLookup(library, arena);
        } catch (IllegalArgumentException ex1) {
            try {
                System.loadLibrary("tree-sitter-zathurarc");
